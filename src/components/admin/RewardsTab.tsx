...
export default function RewardsTab() {
  ...
  const updateItem = (key: string, field: string, value: string | number) => {
    setData((prev) => ({
      ...prev,
      [activeRarity]: prev[activeRarity].map((it) =>
  ...
  const removeItem = (key: string) => {
    setData((prev) => ({
      ...prev,
      [activeRarity]: prev[activeRarity].filter((it) => it._key !== key),
    }));
  };
  ...
  const addItem = () => {
    ...
    setData((prev) => ({
      ...prev,
      [activeRarity]: [...prev[activeRarity], newItem],
    }));
  };

  const handleSave = () => {
    localStorage.setItem('admin_rewards', JSON.stringify(data));
    setSaved(true);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Manage reward items for each rarity tier</p>
      </div>
      ...
      <div className="flex gap-2">
        {(['Common', 'Rare', 'Legendary'] as RewardRarity[]).map((r) => (
          <button key={r} onClick={() => setActiveRarity(r)}
            ...
            style={{
              background: r === activeRarity ? `${rarityColors[r]}20` : 'transparent',
              color: r === activeRarity ? rarityColors[r] : '#64748b',
              border: `1px solid ${r === activeRarity ? `${rarityColors[r]}40` : 'transparent'}`,
            }}>
            {r} ({data[r].length})
          </button>
        ))}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-slate-400 bg-slate-900/40">
                ...
                <th className="text-center py-3 px-4">Currency</th>
                <th className="text-center py-3 px-4">Min</th>
                <th className="text-center py-3 px-4">Max</th>
                <th className="text-center py-3 px-4">Del</th>
              </tr>
            </thead>
            <tbody>
              {items.map((it) => (
                <tr key={it._key} className="border-t" style={{ borderColor: 'rgba(45,212,191,0.06)' }}>
                  <td className="py-2 px-4">
                    <input value={it.icon} onChange={(e) => updateItem(it._key, 'icon', e.target.value)}
                      className="w-12 px-1 py-1 rounded bg-slate-900/60 border border-slate-700/30 text-center text-lg focus:outline-none" />
                  </td>
                  <td className="py-2 px-4">
                    <input value={it.name} onChange={(e) => updateItem(it._key, 'name', e.target.value)}
                      className="w-full px-2 py-1 rounded bg-slate-900/60 border border-slate-700/30 text-teal-200 text-sm focus:outline-none" />
                  </td>
                  <td className="py-2 px-4">
                    <input value={it.description} onChange={(e) => updateItem(it._key, 'description', e.target.value)}
                      className="w-full px-2 py-1 rounded bg-slate-900/60 border border-slate-700/30 text-slate-300 text-sm focus:outline-none" />
                  </td>
                  ...
                  <td className="py-2 px-4">
                    <input type="number" value={it.min ?? ''} onChange={(e) => updateItem(it._key, 'min', Number(e.target.value))}
                      className="w-20 px-2 py-1 rounded bg-slate-900/60 border border-slate-700/30 text-sm text-right text-cyan-300 focus:outline-none" />
                  </td>
                  <td className="py-2 px-4">
                    <input type="number" value={it.max ?? ''} onChange={(e) => updateItem(it._key, 'max', Number(e.target.value))}
                      className="w-20 px-2 py-1 rounded bg-slate-900/60 border border-slate-700/30 text-sm text-right text-cyan-300 focus:outline-none" />
                  </td>
                  <td className="py-2 px-4 text-center">
      ...
      <div className="flex gap-3">
        <button onClick={addItem}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-indigo-500/15 text-indigo-300 border border-indigo-500/25 hover:bg-indigo-500/25 transition-all cursor-pointer">
        ...
        <button onClick={() => { setData(initData()); setSaved(false); }}
